import "@/pages/frontend/app-frontend";
import { errorAPI } from "../../utils/error";

$(document).ready(function () {
    // CEK BROWSER MENDUKUNG WEB STORAGE ?
    if (typeof Storage !== "undefined") {
        // CEK TES AKTIF
        if (localStorage.getItem("ICAREER_TEST_ID") === null) {
            window.location.href = APP_URL + "/icareer";
        }

        let pageNo = "PAGE_NO_ICAREER";
        if (localStorage.getItem(pageNo) === null) {
            localStorage.setItem(pageNo, 1);
        }
        let radioData = [];
        let max_page;
        let modal_time_show = true;

        //# COUNTDOWN
        const countdownText = document.querySelector(".countdown-text");
        const progressBar = document.querySelector(".progress-test");
        const countdownTime = localStorage.getItem("MAX_TIME_ICAREER");
        let timeLeft = localStorage.getItem("timeLeft") || countdownTime;
        let storedWidth = localStorage.getItem("progressWidth") || 0; // Ambil lebar progressBar dari localStorage
        let currentWidth = parseFloat(storedWidth); // Konversi ke tipe float
        const increment = 100 / countdownTime;
        let intervalId;

        function startCountdown() {
            timeLeft = localStorage.getItem("timeLeft") || countdownTime;
            let storedWidth = localStorage.getItem("progressWidth") || 0;
            currentWidth = parseFloat(storedWidth); // Konversi ke tipe float
            clearInterval(intervalId);
            updateCountdown();
        }

        function updateCountdown() {
            currentWidth += increment;
            let sisawaktu = timeLeft.toString().padStart(2, "0");
            countdownText.textContent = sisawaktu;
            progressBar.style.width = `${currentWidth}%`;
            localStorage.setItem("progressWidth", currentWidth); // Simpan lebar progressBar ke localStorage

            if (timeLeft > 0) {
                timeLeft--;
                localStorage.setItem("timeLeft", timeLeft); // Simpan sisa waktu ke localStorage
                intervalId = setTimeout(updateCountdown, 1000);
            } else {
                localStorage.removeItem("timeLeft"); // Hapus data dari localStorage saat waktu habis
                localStorage.removeItem("progressWidth"); // Hapus data progressBar dari localStorage saat waktu habis
                showTimeUpModal();
            }
        }
        // #AKHIR COUNTDOWN

        // # TAMPIL DATA STATEMENT
        function LoadStatement(page = 1) {
            $.ajax({
                url: API_URL + "/api/v1/icareer/statement/member?page=" + page,
                type: "GET",
                timeout: 5000,
                contentType: "application/json",
                success: function (response) {
                    $("#data-icareer-statement").html("");

                    // LOOP DATA TES STATEMENT  DARI API
                    response.data.data.forEach(function (statement, index) {
                        var statementHTML = `
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="text-question">
                                    <p id="statement-value-${index + 1}">${
                            statement.statement
                        }</p>
                                </div>
                            </div>
                        <div class="col-lg-6">
                            <div class="option-group d-flex">
                                <div class="form-check form-check-inline d-flex flex-column">
                                    <input class="form-check-input" type="radio" name="option${
                                        index + 1
                                    }-icareer" value="0"
                                        id="tidak${
                                            index + 1
                                        }-icareer" data-statement-id="${
                            statement.id_statement
                        }">
                                    <label class="form-check-label" for="tidak${
                                        index + 1
                                    }-icareer" id="tidak${
                            index + 1
                        }-icareer-label">
                                        Tidak
                                    </label>
                                </div>
                                <div class="form-check form-check-inline d-flex flex-column">
                                    <input class="form-check-input" type="radio" name="option${
                                        index + 1
                                    }-icareer" value="1"
                                        id="kurang${
                                            index + 1
                                        }-icareer" data-statement-id="${
                            statement.id_statement
                        }">
                                    <label class="form-check-label" for="kurang${
                                        index + 1
                                    }-icareer" id="kurang${
                            index + 1
                        }-icareer-label">
                                        Kurang
                                    </label>
                                </div>
                                <div class="form-check form-check-inline d-flex flex-column">
                                    <input class="form-check-input" type="radio" name="option${
                                        index + 1
                                    }-icareer" value="2"
                                        id="cukup${
                                            index + 1
                                        }-icareer" data-statement-id="${
                            statement.id_statement
                        }" >
                                    <label class="form-check-label" for="cukup${
                                        index + 1
                                    }-icareer" id="cukup${
                            index + 1
                        }-icareer-label">
                                        Cukup
                                    </label>
                                </div>
                                <div class="form-check form-check-inline d-flex flex-column">
                                    <input class="form-check-input" type="radio" name="option${
                                        index + 1
                                    }-icareer" value="3"
                                        id="sesuai${
                                            index + 1
                                        }-icareer" data-statement-id="${
                            statement.id_statement
                        }">
                                    <label class="form-check-label" for="sesuai${
                                        index + 1
                                    }-icareer" id="sesuai${
                            index + 1
                        }-icareer-label">
                                        Sesuai
                                    </label>
                                </div>
                            </div>
                        </div>
                            <hr class="hr-test-icareer mx-auto">
                        </div>`;

                        $("#data-icareer-statement").append(statementHTML);
                    });

                    // Bersihkan data di radioData
                    radioData = [];
                    $("#btn-selanjutnya").prop("disabled", true);
                    $("#page-test-link").text(
                        "Halaman " + localStorage.getItem("PAGE_NO_ICAREER")
                    );

                    // Start Countdown
                    startCountdown();

                    max_page = response.data.max_page;
                    // TAMPILKAN DATA INPUT DARI LOCAL STORAGE
                    displayStoredCheckboxData();
                },
                error: function (xhr, status, error) {
                     errorAPI(xhr,status,error)
                },
            });
        }

        // JAWABAN TES DI STORAGE
        function displayStoredCheckboxData() {
            let storedRadioData = localStorage.getItem("radioData");
            if (storedRadioData !== null) {
                radioData = JSON.parse(storedRadioData);

                radioData.forEach(function (data) {
                    let radioInput = $(
                        `input[name='${data.name}'][data-statement-id='${data.statementId}'][value='${data.value}']`
                    );
                    if (radioInput.length > 0) {
                        radioInput.prop("checked", true);
                    }
                });
            }

            if (radioData.length == 6) {
                $("#btn-selanjutnya").prop("disabled", false);
            }
        }

        // TIME UP MODAL
        function showTimeUpModal() {
            const timeUpModal = new bootstrap.Modal(
                document.getElementById("timeUpModal"),
                {}
            );
            if (modal_time_show == true) {
                timeUpModal.show();
            }
        }

        //# HANDLE SIMPAN DATA INPUT
        $("body").on("click", 'input[type="radio"]', function () {
            var radioValue = $(this).val();
            var radioName = $(this).attr("name");
            var statementId = $(this).attr("data-statement-id");

            var existingData = radioData.find(
                (item) => item.name === radioName
            );

            if (existingData) {
                existingData.value = radioValue;
                existingData.statementId = statementId; // Tambahkan statementId jika sudah ada data yang sesuai
            } else {
                //radioData  = menyimpan data checkbox yg di click
                radioData.push({
                    name: radioName,
                    value: radioValue,
                    statementId: statementId,
                });
            }

            // SIMPAN DATA KE LOCAL STORAGE
            localStorage.setItem("radioData", JSON.stringify(radioData));

            // SIMPAN KE API
            let data = {
                test_id: localStorage.getItem("ICAREER_TEST_ID"),
                statement_id: statementId,
                range_choice: radioValue,
            };
            $.ajax({
                url: API_URL + "/api/v1/icareer/user-input",
                type: "POST",
                data: JSON.stringify(data),
                timeout: 5000,
                contentType: "application/json",
                success: function (response) {},
                error: function (xhr, status, error) {
                     errorAPI(xhr,status,error)
                },
            });

            if (radioData.length == 6) {
                $("#btn-selanjutnya").prop("disabled", false);
            }
        });

        // HANDLE SOAL TES SELANJUTNYA
        $("body").on("click", "#btn-selanjutnya", function () {
            if (localStorage.getItem("PAGE_NO_ICAREER") == max_page) {
                const successModal = new bootstrap.Modal(
                    document.getElementById("successModal"),
                    {}
                );
                modal_time_show = false;
                successModal.show();
            } else {
                // LOAD DATA SELANJUTNYA
                let count = localStorage.getItem(pageNo);
                count++;
                LoadStatement(count);
                localStorage.setItem(pageNo, count);
                localStorage.removeItem("radioData");

                // Mulai kembali countdown dari awal
                timeLeft = countdownTime;
                localStorage.setItem("timeLeft", timeLeft); // Set ulang sisa waktu ke nilai awal
                currentWidth = 0;
                localStorage.setItem("progressWidth", currentWidth); // Set ulang lebar progressBar ke nilai awal
                startCountdown();
            }
        });

        //HANDLE WAKTU HABIS
        $("body").on("click", "#btn-waktu-habis", function () {
            startCountdown();
        });

        // HANDLE CLOSE MODAL
        let timeUpModal = document.getElementById("timeUpModal");
        timeUpModal.addEventListener("hidden.bs.modal", function (event) {
            startCountdown();
        });
        let successModal = document.getElementById("successModal");
        successModal.addEventListener("hidden.bs.modal", function (event) {
            startCountdown();
            modal_time_show = true;
        });

        // # HANDLE BTN TES SELESAI
        $("body").on("click", "#btn-lihat-hasil", function (e) {
            e.preventDefault();
            let testId = localStorage.getItem("ICAREER_TEST_ID");

            // HANDLE SIMPAN SCORE
            $.ajax({
                url: API_URL + "/api/v1/icareer/save-score/" + testId,
                type: "PUT",
                timeout: 5000,
                success: function (response) {
                    // Hapus data dari localStorage saat waktu habis
                    localStorage.removeItem("timeLeft");
                    localStorage.removeItem("progressWidth");
                    localStorage.removeItem("PAGE_NO_ICAREER");
                    localStorage.removeItem("radioData");
                    localStorage.removeItem("MAX_TIME_ICAREER");
                    localStorage.removeItem("ICAREER_TEST_ID");

                    // pindah halaman
                    window.location.href = APP_URL + "/assesment";
                },
                error: function (xhr, status, error) {
                     errorAPI(xhr,status,error)
                },
            });
        });

        // LOAD DATA & START COUNTDOWN
        LoadStatement(localStorage.getItem("PAGE_NO_ICAREER"));
    } else {
        alert("Browser yang Anda gunakan tidak mendukung Web Storage");
    }
});
