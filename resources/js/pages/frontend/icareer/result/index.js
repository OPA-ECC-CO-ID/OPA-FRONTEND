...
$(document).ready(function () {
    ...
    if (localStorage.getItem("ICAREER_TEST_ID")) {
            ...
            success: function (response) {
                ...
                if (response.data.icareer === "Lihat Hasil") {
                    ...
                    $.ajax({
                        ...
                        contentType: "application/json",
                        success: function (response) {
                            console.log(response.data.evaluation_grade);
                            ...
                            if (isAfterToday) {
                                // belum 3 bulan
                                let elementData = `<p>Kamu sudah melakukan tes pada tanggal ${time_test}. Tes ini hanya dapat diikuti selama 3 bulan sekali. Kamu dapat mengikuti tes sekali lagi mulai tanggal ${next_test}.</p>`;
                            ...
                            let resultContainer = $("#hasil-asesment");
                            let resultBoxHTML;
                            response.data.variable.forEach((result) => {
                                resultBoxHTML += `
                                <tr>
                                    <th scope="row">${
                            ...
                            // var global = [];
                            // response.data.title.forEach((result) => {
                            //     global.push(result);
                            // });
                            // var combineGlobal = global.join(", ");
                            // $("#global-minat-karir").text(combineGlobal);
                            $("#global-minat-karir").text(response.data.title);

                            console.log(response.data);
                            // Tampil data survei pengalaman
                            var radioElement = document.querySelector(
                                'input[name="rating"][value="' +response.data.evaluation_grade +'"]'
                            );
                            if (radioElement) {
        ...
        $('#btn-simpan').on('click',function() {
            ...
            $.ajax({
                url: `${API_URL}/api/v1/icareer/tests/${user_id}/survey`,
                type: 'PUT',
                contentType: 'application/json',
