...
$(document).ready(function () {
    $("#myTable").DataTable({
        ...
        serverside: true,
        ajax: {
            url: API_URL + "/api/v1/ipedia/universities?limit=-1",
            type: "GET",
            dataSrc: "data.data",
            timeout: 5000,
    ...
    $("#myTrashTable").DataTable({
        ...
        serverside: true,
        ajax: {
            url: API_URL + "/api/v1/ipedia/universities/deleted?limit=-1",
            type: "GET",
            dataSrc: "data.data",
            timeout: 5000,
            error: function (xhr, status, error) {
        ...
        columns: [
            ...
            },
            {
                render: function (data, type, full, meta) {
                    ...
                    return `
                    <div class="action-table">
                        <button class="badge bg-gradient-info tombol-restore" data-id="${id}" data-bs-toggle="modal"><i class="fa fa-undo" aria-hidden="true"></i></button>
...
$("body").on("click", "#btn-show-trash-universities", function () {
    $("#btn-show-trash-universities").addClass("bg-gradient-danger");
    $("#data_universities").hide();
    $("#data_trash_universities").show();
    $(".tombol-tambah").hide();
    // hapus warna btn
    $("#btn-show-universities").removeClass("bg-gradient-primary");
});

// HANDLE TAMPIL DATA
$("body").on("click", "#btn-show-universities", function () {
    $("#btn-show-universities").addClass("bg-gradient-primary");
    $("#data_universities").show();
    $("#data_trash_universities").hide();
    $(".tombol-tambah").show();
    // hapus warna btn
    $("#btn-show-trash-universities").removeClass("bg-gradient-danger");
});
...
$("body").on("click", ".tombol-edit", function () {
    ...
    // #2 Tampilkan Data
    $.ajax({
        url: API_URL + "/api/v1/ipedia/universities/" + id,
        type: "GET",
        success: function (response) {
            ...
            $("#university_name").val(response.data.university_name);
...
$("body").on("click", ".tombol-restore", function (e) {
    ...
    $.ajax({
        url: API_URL + "/api/v1/ipedia/universities/restore",
        type: "POST",
        dataType: "json",
...
$("body").on("click", ".tombol-force-delete", function (e) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            var id = $(this).data("id");
            $.ajax({
                url: API_URL + "/api/v1/ipedia/universities/force-delete/" + id,
                type: "DELETE",
                success: function (response) {
                    Swal.fire(
                        "Deleted!",
...
function simpan(id = "") {
    ...
    // #2. Cek Data
    if (id == "") {
        var_url = API_URL + "/api/v1/ipedia/universities";
        type_url = "POST";
        ...
        var_url = API_URL + "/api/v1/ipedia/universities/" + id;
        type_url = "PUT";
