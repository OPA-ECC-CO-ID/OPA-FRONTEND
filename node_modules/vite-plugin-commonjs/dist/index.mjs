...
import { Resolve, globFiles, mappingPath } from "vite-plugin-dynamic-import";
...
function generateImport(analyzed, id, rules) {
  const imports = [];
  let count = 0;
...
async function transformCommonjs({
  ...
  let ast;
  try {
    ast = parse(code, { sourceType: "module", ecmaVersion: 14 });
  } catch (error) {
    return null;
  }
  const analyzed = analyzer(ast, code, id);
  const imports = generateImport(analyzed, id, (_b = options.advanced) == null ? void 0 : _b.importRules);
